...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        for i in range(len(predict_examples)):
            labels.append(int(predict_examples[i].label))
            text_a.append(predict_examples[i].text_a)
            text_b.append(predict_examples[i].text_b)
        # print(labels[:10])
        # print(prediction)
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for i in range(len(predictions)):
                ...
                output_line = "%s\t%d\t%d\n" % (text_a[i], labels[i], predictions[i])
